	...
	for s in sample_dict:
		...
		with open(log_filename, "a") as log_file:
			...
			if cfg.use_different_reference_for_each_sample == True:
				...
				call("varscan mpileup2snp {s}/{sam_file}.pileup --min-coverage {min_cov} --min-avg-qual {snp_qual_threshold} --min-var-freq {snp_frequency} --strand-filter {strand_filter} --output-vcf 1 > {s}/{vcf_name}".format(s=s, vcf_name=vcf_name, sam_file=sam_file, snp_frequency=cfg.snp_frequency,min_cov=cfg.min_cov, snp_qual_threshold=cfg.snp_qual_threshold,strand_filter=cfg.strand_filter), shell=True, stderr=log_file)
